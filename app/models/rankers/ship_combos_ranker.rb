module Rankers
  class ShipCombosRanker
    ...
    def initialize(ranking_configuration, ship_id: nil, ship_combo_id: nil, pilot_id: nil, upgrade_id: nil, limit: nil)
      ...
      @ships       = Hash[ShipCombo.where(id: @ship_combos.map(&:id)).includes(:ships).map { |c| [c.id, c.ships.map { |s| { id: s.id, xws: s.xws, name: s.name, font_icon_class: s.font_icon_class } }] }]
